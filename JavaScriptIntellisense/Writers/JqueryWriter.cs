...
{
    public class JqueryWriter : IWriter
    {
        ...
        public void StartWriting(List<Entry> list, StringBuilder sb)
        {
            ...
            Write(list.Where(e => e.Name.StartsWith("jQuery.") && !e.Name.Contains("fx.")), "jQuery", sb);
            Write(list.Where(e => e.Name.StartsWith("callbacks.")), "jQuery.Callbacks", sb);
            Write(list.Where(e => e.Name.StartsWith("deferred.")), "jQuery.Deferred", sb);
            Write(list.Where(e => e.Name.StartsWith("event.")), "jQuery.Event.prototype", sb);
            Write(list.Where(e => !e.Name.Contains(".") && e.Name != "jQuery"), "jQuery.fn", sb);

            var constructor = list.Single(e => e.Name == "init");
            constructor.Name = "$";

            Write(new[] { constructor }, "window", sb);
        }
        ...
        private void Write(IEnumerable<Entry> entries, string objectName, StringBuilder sb)
        {
            ...
            if (objectName == "jQuery.Callbacks") { WriteCallbacks(objectName, "flags", sb, hash); }
            else if (objectName == "jQuery.Deferred") { WriteCallbacks(objectName, "func", sb, hash); }
            else
            {
